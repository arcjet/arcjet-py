...
│   ├── _component.py                     # AnalyzeComponent with 6 typed methods (GENERATED)
...
a custom code generator that reads WIT extracted from the WASM binary and
produces 5 Python source files. The generator lives in `tools/witgen/` and is
configured by `witgen.toml` at the repo root.

```sh
# Regenerate bindings after updating the WASM binary:
uv run python -m tools.witgen
uv run ruff check --select I --fix && uv run ruff format
uv run pytest arcjet-analyze/tests/ --no-cov
```

Generated files: `_types.py`, `_convert.py`, `_component.py`, `_imports.py`,
`__init__.py`. The only non-generated file is `_import_defaults.py` (domain-
specific default callback implementations).
...
`AnalyzeComponent` uses a per-instance `threading.Lock` around `_call()`.
