...
``import arcjet.guard`` must keep working. :func:`guard_tool` is a
callback and does not load the peer. :func:`guard_plugin` subclasses
``BasePlugin``, so the first call that builds a plugin loads the peer
and says what to install if it is missing or too old.

The floor is 2.0.0 — the first 2.x whose Runner ``BasePlugin``
``before_tool_callback`` skips the tool with a returned dict. Verified
on 2.0.0. The default extra set does not pull chromadb.
...
#: First 2.x whose Runner ``BasePlugin.before_tool_callback`` skips the
#: tool with a returned dict. Verified on 2.0.0.
...
def _require_google_adk() -> None:
    """Refuse a peer too old to skip a tool, before anything is wired.

    Below the floor there is no 2.x ``BasePlugin.before_tool_callback``
    skip-dict path this adapter can honestly use. Reported at plugin-build
    time rather than as an ``AttributeError`` from inside a callback the
    SDK would propagate. A missing install is left to the import path.
    """
    ...
    if release and release < MINIMUM_GOOGLE_ADK:
        ...
        raise ArcjetMisconfiguration(
            ...
            f"{installed} is installed. Below {floor} there is no 2.x "
            f"BasePlugin.before_tool_callback skip-dict path. Upgrade "
...
def load_base_plugin() -> type[Any]:
    """Google ADK's ``BasePlugin``, or an error naming what to install."""
    _require_google_adk()
    try:
...
def google_adk_present() -> bool:
    """Whether the ``google.adk`` package is importable in this process."""
    try:
