...
def test_converting_prompt_injection_reason() -> None:
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert reason.score == 0.95  # pyright: ignore[reportDeprecated]
    assert any(
        issubclass(x.category, DeprecationWarning) and "score" in str(x.message)
        for x in w
    )
...
def test_converting_prompt_injection_reason_not_detected() -> None:
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        assert reason.score == 0.2  # pyright: ignore[reportDeprecated]
    assert any(
        issubclass(x.category, DeprecationWarning) and "score" in str(x.message)
        for x in w
    )
